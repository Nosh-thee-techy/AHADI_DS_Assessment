...
def _render() -> None:
    ...
    with st.container(key="filter_bar"):
        lang_col, theme_col, year_col, sex_col, map_col, county_col, compare_col = st.columns(
            (1.2, 1.0, 0.7, 0.85, 1.35, 1.15, 1.15),
            gap="small",
        )
        with lang_col:
        ...
        mix_col, dl_col, _ = st.columns((1.6, 1.1, 4.3), gap="small")
        with mix_col:
